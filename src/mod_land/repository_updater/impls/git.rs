...
use crate::mod_land::repository_updater::{RepositoryUpdater, error};

#[cfg(feature = "git2")]
pub(in crate::mod_land) struct Git {}
...
impl Git {
  ...
  fn can_update(&self, url: &Url) -> bool {
    matches!(url.scheme(), "git+http" | "git+https")
  }

  async fn init_repo(&self, url: Url, path: &Path) -> Result<(), error::InitRepo> {
    if fs::try_exists(path)
      .await
      .expect("check init repo path failed")
    {
...
#[async_trait]
impl RepositoryUpdater for Git {
  fn can_update(&self, url: &Url) -> bool {
    self.can_update(url)
  }

  async fn init_repo(&self, url: Url, path: &Path) -> Result<(), error::InitRepo> {
    self.init_repo(url, path).await
  }

  async fn update_repo(&self, _url: Url, path: &Path) -> Result<(), error::UpdateRepo> {
    self.update_repo(path).await
  }
}

// from https://github.com/rust-lang/git2-rs/blob/e8c9126d90d4dfedf0fdd0ffb355baf36a4899f3/examples/pull.rs#L1
fn do_fetch<'a>(
